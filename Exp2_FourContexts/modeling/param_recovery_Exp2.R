...
sims <- lapply(1:n_sims, function(i) {
  sim <- simFunction(model, modeling_data[[indices[i]]], theta=if(free_theta) NULL else 50, delta=0)
  sim(params[i,])
})
...
system.time(
  fits <- lapply(1:n_sims, function(i) {
    ...
    data[['RT']] <- sims[[i]][,2]
    objFun <- objectiveFunction(model, data, theta=if(free_theta) NULL else 50, delta=0)
