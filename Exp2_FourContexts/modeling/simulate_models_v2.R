...
# load in the RL-LBA model functions (v2)
...
model1_fits <- readRDS('results/model1_fits_v2.RDS')
model2_fits <- readRDS('results/model2_fits_v2.RDS')
model3_fits <- readRDS('results/model3_fits_v2.RDS')
model4_fits <- readRDS('results/model4_fits_v2.RDS')
model5_fits <- readRDS('results/model5_fits_v2.RDS')
model6_fits <- readRDS('results/model6_fits_v2.RDS')
model7_fits <- readRDS('results/model7_fits_v2.RDS')
...
model1_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model_v2(params1[i,], version=1, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                   gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})

model2_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model_v2(params2[i,], version=2, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                   gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})

model3_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model_v2(params3[i,], version=3, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                   gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})

model4_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model_v2(params4[i,], version=4, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                   gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})

model5_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model_v2(params5[i,], version=5, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                   gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})

model6_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model_v2(params6[i,], version=6, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                   gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})

model7_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model_v2(params7[i,], version=7, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                   gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})
...
# saveRDS(model1_sims, file='results/model1_sims_v2.RDS')
# saveRDS(model2_sims, file='results/model2_sims_v2.RDS')
# saveRDS(model3_sims, file='results/model3_sims_v2.RDS')
# saveRDS(model4_sims, file='results/model4_sims_v2.RDS')
# saveRDS(model5_sims, file='results/model5_sims_v2.RDS')
# saveRDS(model6_sims, file='results/model6_sims_v2.RDS')
# saveRDS(model7_sims, file='results/model7_sims_v2.RDS')
