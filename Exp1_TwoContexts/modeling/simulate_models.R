...
recode_simulated_choices <- function(datasets, simulations, n_sims) {
  ...
  for (i in 1:length(datasets)) {
    for (j in 1:n_sims) {
      simulations[[i]][,1,j] <- 1*(((datasets[[i]]$EV_1 > datasets[[i]]$EV_2) & (simulations[[i]][,1,j] == 1)) |
                                     ((datasets[[i]]$EV_1 < datasets[[i]]$EV_2) & (simulations[[i]][,1,j] == 2))) +
        0*(((datasets[[i]]$EV_1 > datasets[[i]]$EV_2) & (simulations[[i]][,1,j] == 2)) |
...
model1_sims <- lapply(1:length(datasets), function(i) {
  replicate(n=100, RL_LBA_Model(params1[i,], version=1, options=datasets[[i]]$options, outcomes=datasets[[i]]$outcomes,
                                avail=datasets[[i]]$avail, gaze=datasets[[i]]$gaze_pre, simulate=TRUE))
})
