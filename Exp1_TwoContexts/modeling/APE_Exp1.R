...
w_rel <- as.double(args[3])
free_theta <- as.integer(args[4])
...
if ( free_theta ) {
  system.time(
    results <- lapply(batches[[batch]], function(index) {
      ...
      llvals <- computeAPE(model, modeling_data[[index]], w_rel=w_rel, delta=0, from=s, to=N, 
  ...
  system.time(
    results <- lapply(batches[[batch]], function(index) {
      ...
      llvals <- computeAPE(model, modeling_data[[index]], w_rel=w_rel, theta=50, delta=0, from=s, to=N, 
                           lower_bounds=lower_bounds, upper_bounds=upper_bounds,
