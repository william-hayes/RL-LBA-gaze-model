...
model8_lc <- sapply(sims8, function(X) rowMeans(X[,1,]), simplify='array')
...
PLOT FOR MAIN MANUSCRIPT (Fig 4A)
...
model8_rtc <- sapply(sims8, function(X) rowMeans(X[,2,]), simplify='array')
...
PLOT FOR MAIN MANUSCRIPT (Fig 4B)
...
PLOT FOR SI APPENDIX (S6 Fig)

```{r}
# png('Exp1_ChoiceRT_All_Models.png', width=11.5, height=5, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
...
model8_acc <- sapply(sims8, function(X) mean(X[,1,]))
...
PLOT FOR MAIN MANUSCRIPT (Fig 4C)
...
5. Relationships with model parameters (S1 Table)

```{r}
# load model 7 fits
model7_fits <- readRDS('../modeling/results/free-theta/model7_fits.RDS')

# extract parameter estimates
params <- data.frame(t(sapply(model7_fits, function(X) X$optim$bestmem)))
names(params) <- c('t0','learn_rate','Q_beta','gaze_beta','threshold_sep','upper_bound','theta')
...
summary(lm(acc ~ learn_rate + Q_beta + gaze_beta + theta + upper_bound + threshold + t0, data=accData))
```
...
model8_rt <- sapply(sims8, function(X) mean(X[,2,]))
...
PLOT FOR MAIN MANUSCRIPT (Fig 4D)
...
PLOT FOR SI APPENDIX (S8 Fig)

```{r}
# png('Exp1_IndividChoiceRT_All_Models.png', width=11.5, height=5, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
# grid.arrange(p1, p2, nrow=2, ncol=1)
# dev.off()
```
...
5. Relationships with model parameters (S3 Table)
...
summary(lm(mean_RT ~ learn_rate + Q_beta + gaze_beta + theta + upper_bound + threshold + t0, data=rtData))
```
...
3. Plot correct and incorrect RT distributions with model simulations overlaid (SI APPENDIX, S10 Fig)
...
model7_accByGaze <- sapply(1:length(sims7), function(i) apply(sims7[[i]][,1,], 2, FUN=function(x_j) {
  aggregate(x_j ~ factor(datasets[[i]]$gaze_bucket), FUN=mean)[,2]}), simplify='array')
model8_accByGaze <- sapply(1:length(sims8), function(i) apply(sims8[[i]][,1,], 2, FUN=function(x_j) {
  aggregate(x_j ~ factor(datasets[[i]]$gaze_bucket), FUN=mean)[,2]}), simplify='array')
...
PLOT FOR MAIN MANUSCRIPT (Fig 5A)
...
model7_RTByGaze <- sapply(1:length(sims7), function(i) apply(sims7[[i]][,2,], 2, FUN=function(x_j) {
  aggregate(x_j ~ factor(datasets[[i]]$gaze_bucket), FUN=mean)[,2]}), simplify='array')
model8_RTByGaze <- sapply(1:length(sims8), function(i) apply(sims8[[i]][,2,], 2, FUN=function(x_j) {
  aggregate(x_j ~ factor(datasets[[i]]$gaze_bucket), FUN=mean)[,2]}), simplify='array')
...
PLOT FOR MAIN MANUSCRIPT (Fig 5B)
...
PLOT FOR SI APPENDIX (S13 Fig)

```{r}
# png('Exp1_GazeEffects_All_Models.png', width=11.5, height=5, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
# grid.arrange(p1, p2, nrow=2, ncol=1)
# dev.off()
```
...
PLOT FOR MAIN MANUSCRIPT (Fig 5C)
...
PLOT FOR MAIN MANUSCRIPT (Fig 5D)
...
## Model comparison v1: relative encoding parameter set to 0
...
# trim off first five trials for Models 1-7 (all zeros since APE was computed based on trials 6-60)
...
# compute difference in APE relative to Model 7 w/ theta parameter free
...
APE_data <- data.frame(model = rep(model_order, each=N),
                       ...
                       APE_diff = c( colSums( (-model1_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model2_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model3_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model4_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model4FT_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model5_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model5FT_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model6_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model6FT_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model7_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model7FT_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model8_LL) - (-model7FT_LL) , na.rm=TRUE),
                                     colSums( (-model8FT_LL) - (-model7FT_LL) , na.rm=TRUE)))
...
# par(mar=c(4.5,1,1.5,1))
# ggplot(subset(APE_diff_data, version %in% c('no theta', 'theta free')),
#        aes(x=model, y=mean_APE_diff)) +
...
#   geom_text(aes(y=upper+0.5, label=label)) +
#   ggtitle('Exp. 1')
# dev.off()
```
...
```{r}
# png('APE_Exp1_extended_wrel0.png', width=6.5, height=3.5, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
# ggplot(APE_diff_data, aes(x=model, y=mean_APE_diff, fill=version)) +
#   geom_hline(yintercept = 0, lty=2) +
#   geom_col(col='black', position=position_dodge(width=0.6), width=0.5) +
#   geom_errorbar(position=position_dodge(width=0.6), aes(ymin=lower, ymax=upper), width=0.2) +
#   ylab(expression(paste(Delta, 'APE (model i ', '\U2212', ' model 7 [theta free])'))) +
#   xlab('Model') +
#   theme_classic(base_size=11) +
#   theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1),
#         plot.title = element_text(hjust=0.5, size=11)) +
#   scale_y_continuous(limits=c(0, 16), breaks=seq(0, 16, 2), expand=c(0,0)) +
#   geom_text(aes(y=upper+0.8, label=label), position=position_dodge(width=0.6)) +
#   ggtitle(expression(paste('Exp. 1 (', italic(w)[italic('rel')], ' = 0)', sep=''))) +
...
```{r}
with(APE_data, t.test(APE_diff[model=='model_8FT']))
```



## Model comparison v2: relative encoding parameter set to 1

1. Load accumulative one-step-ahead prediction error (APE) results for each model. These were run in batches.

```{r}
model1_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model1_wrel1_batch', i, '_APE.RDS'))
})

model2_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model2_wrel1_batch', i, '_APE.RDS'))
})

model3_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model3_wrel1_batch', i, '_APE.RDS'))
})

model4_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model4_wrel1_batch', i, '_APE.RDS'))
})

model5_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model5_wrel1_batch', i, '_APE.RDS'))
})

model6_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model6_wrel1_batch', i, '_APE.RDS'))
})

model7_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model7_wrel1_batch', i, '_APE.RDS'))
})

model8_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/model8_wrel1_batch', i, '_APE.RDS'))
})


model4FT_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/free-theta/model4_wrel1_batch', i, '_APE.RDS'))
})

model5FT_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/free-theta/model5_wrel1_batch', i, '_APE.RDS'))
})

model6FT_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/free-theta/model6_wrel1_batch', i, '_APE.RDS'))
})

model7FT_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/free-theta/model7_wrel1_batch', i, '_APE.RDS'))
})

model8FT_APE_results <- lapply(1:20, FUN=function(i) {
  readRDS(paste0('../modeling/results/free-theta/model8_wrel1_batch', i, '_APE.RDS'))
})
...
PLOT FOR SUPPLEMENTAL INFO WITH EXTENDED SET OF MODELS

```{r}
# png('APE_Exp1_extended_wrel1.png', width=6.5, height=3.5, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
# ggplot(APE_diff_data, aes(x=model, y=mean_APE_diff, fill=version)) +
#   geom_hline(yintercept = 0, lty=2) +
...
#   scale_y_continuous(limits=c(0, 16), breaks=seq(0, 16, 2), expand=c(0,0)) +
#   geom_text(aes(y=upper+0.8, label=label), position=position_dodge(width=0.6)) +
#   ggtitle(expression(paste('Exp. 1 (', italic(w)[italic('rel')], ' = 1)', sep=''))) +
#   scale_fill_manual(values=c('no theta' = 'white',
#                              'theta = 50' = 'grey80',
#                              'theta free' = 'grey40')) +
#   guides(fill=guide_legend(title=''))
...
```{r}
# png('Exp1_model_prediction_accuracy.png', width=2, height=2.5, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
...
```{r}
# png('Exp1_model_prediction_accuracy_by_gaze.png', width=6, height=2.5, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
# ggplot(modelPredAccData2, aes(x=model, y=mean_accuracy)) +
...
## Individual differences (S3 Appendix)
...
param_names <- c('Learning rate', 'Q drift scaling', 'Gaze drift scaling', 'Softmax inv. temp.',
...
paramRecoveryData <- data.frame(
  ...
  generating = c(paramRecoveryResults[['generating']][,c(2,3,4,7,6,8,1)]),
  recovered = c(paramRecoveryResults[['recovered']][,c(2,3,4,7,6,8,1)])
)
```


2. Plot relationships between generating and recovered parameters (SI APPENDIX, S4 Fig)
...
p1 <- ggplot(data=paramRecoveryData, aes(x=generating, y=recovered)) +
  ...
  ggtitle('Experiment 1', subtitle='softmax(Q) + gaze model') 
```
...
```{r}
# png('Exp1_param_recovery_R1.png', width=7, height=8, units='in', res=300)
# par(mar=c(4.5,1,1.5,1))
# grid.arrange(p1, p2, nrow=2, ncol=1)
# dev.off()
```
